...
def model_run(train,test,type_name):
    ...
    summary_df.to_excel('回归系数表及列线图数据集//seed0'+type_name+'因子系数表(无逐步回归).xlsx')
...
def dca(data,day,type_name):
    ...
    for var in ['既往史-颌面部放射线接触史','个人史-吸烟史','专科检查-原发肿瘤主体部位']:
        tmp_df = pd.get_dummies(data[var],prefix = var.split('-')[0],drop_first = True)
        data = pd.concat([data,tmp_df],axis = 1)
    data = data.drop(['既往史-颌面部放射线接触史','个人史-吸烟史','专科检查-原发肿瘤主体部位'],axis = 1)
    n = data.shape[0]
...
def Decision_Curve(test,predict_prob,days):
    n = test.shape[0]
    thresh_list = np.arange(0,1,0.001)
...
with PdfPages('决策曲线图.pdf') as pdf:
    ...
    plt.plot(thresh_list,benfit_list_other,label = 'Clinical prognostic model')
    plt.plot(thresh_list,benfit_list_tn,label = 'Pathological prognostic model')
